...
class ConsoleKernel implements ConsoleKernelContract
{
    ...
    public function getArtisan(): ConsoleApplicationContract|ConsoleApplication
    {
        app()->alias(
            ...
            ConsoleApplicationContract::class
        );
        app()->alias(
            \Illuminate\Console\Application::class,
        ...
        return $this->artisan ??= app()->instance(
            \Illuminate\Console\Application::class,
            new class extends ConsoleApplication implements
                ConsoleApplicationContract
            {
                /**
                ...
                public function call(
                    ...
                    $command = class_exists($command)
                        ? app()->make($command)->getName()
                        : $command;
                    ...
                    if (!$this->has($command)) {
                        throw new \Symfony\Component\Console\Exception\CommandNotFoundException(
                            sprintf(
                ...
                public function output()
                {
                    return $this->lastOutput &&
                        method_exists($this->lastOutput, "fetch")
    ...
    protected function commands()
    {
        ...
        Cache::store("installation")->set(
            "commands.core",
            collect(glob(APP_ROOT . "/app/Command/*.php") ?? [])
                ->filter(function ($command) use (&$customCommands) {
                    return !Arr::has(
                        $customCommands ??= collect(
                            glob(APP_ROOT . "/custom/Command/*.php") ?? []
                        ),
                        str_replace(
                            APP_ROOT . "/app",
                            APP_ROOT . "/custom",
        ...
        Cache::store("installation")->set(
            "commands.plugins",
            collect(glob(APP_ROOT . "/app/Plugins/*/Command/*.php") ?? [])
                ->filter(function ($command) use (&$customPluginCommands) {
                    return !in_array(
                        ...
                        $customPluginCommands ??= collect(
                            glob(
                                APP_ROOT . "/custom/Plugins/*/Command/*.php"
                            ) ?? []
                        )->toArray()
                    );
                })
                ...
                ->filter(
                    fn($command) => in_array(
                        Str::of($command)
                            ...
                            ->toString(),
                        array_map(
                            fn($plugin) => $plugin->foldername,
                            $this->getApplication()
                                ->make(PluginsService::class)
                                ->getAllPlugins(enabledOnly: true)
                        )
                    )
                )
        );
        ...
        $commands = collect(
            Arr::flatten(
                Cache::store("installation")->many([
                    "commands.core",
                    "commands.plugins",
                ])
            )
        )->map(
            fn($path) => $this->getApplication()->getNamespace() .
                Str::of($path)
                    ->remove([APP_ROOT . "/app/", APP_ROOT . "/custom/"])
                    ->replace(["/", ".php"], ["\\", ""])
                    ->toString()
        );
        ...
        $laravelCommands = collect($glob)->map(function ($command) {
            $path = Str::replace(
                APP_ROOT . "/vendor/illuminate/",
                "",
                $command
            );
            $cleanPath = ucfirst(
                Str::replace(["/", ".php"], ["\\", ""], $path)
            );
    ...
    private function schedule()
    {
        ...
        app()->singleton(Schedule::class, function ($app) {
            $schedule = tap(
                new Schedule($app["config"]["defaultTimezone"])
