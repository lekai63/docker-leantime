...
class Get extends Controller
{
     ...
     */
    private function getFileLocally(
        $encName,
        $ext,
        $module,
        $realName
        ...
        if (file_exists(realpath($fullPath))) {
            if ($fd = fopen(realpath($fullPath), "rb")) {
                ...
                if (app()->make(Environment::class)->debug == false) {
                    ...
                    $oStreamResponse->headers->set(
                        "Cache-Control",
                        "max-age=86400"
                    );
                    $oStreamResponse->headers->set(
                        "Last-Modified",
                        gmdate("D, d M Y H:i:s", $sLastModified) . " GMT"
                    );
                } else {
    ...
    private function getFileFromS3($encName, $ext, $module, $realName): Response
    {
        ...
        try {
            ...
            if ($ext == "pdf") {
                ...
                $response->headers->set(
                    "Content-Disposition",
                    'attachment; filename="' . $realName . "." . $ext . '"'
                );
            }
